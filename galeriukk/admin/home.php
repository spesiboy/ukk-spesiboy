...
<div class="container mt-3">
    ...
    <div class="row">
        ...
        if (isset($_GET['albumid'])) {
            ...
            $query = mysqli_query($koneksi, "SELECT foto.*, user.namalengkap, album.namaalbum FROM foto INNER JOIN user ON foto.userid=user.userid INNER JOIN album ON foto.albumid=album.albumid WHERE foto.userid='$userid' AND foto.albumid='$albumid'");
                    ...
                    <div class="card text-white bg-success mb-3" style="max-width: 18rem;">
                        ...
                        <div class="card-footer text-center">
                            ...
                            if (mysqli_num_rows($ceksuka) == 1) { ?>
                                <a href="../config/proses_like_copy.php?fotoid=<?php echo $data['fotoid'] ?>" type="submit" name="batalsuka"><i class="fas fa-heart" style="color: red;"></i></a>

                            <?php } else { ?>
                                <a href="../config/proses_like_copy.php?fotoid=<?php echo $data['fotoid'] ?>" type="submit" name="suka"><i class="fa-regular fa-heart"></i></a>

                            <?php }

                            $like = mysqli_query($koneksi, "SELECT * FROM likefoto WHERE fotoid='$fotoid'");
                            echo mysqli_num_rows($like) . ' 喜欢';
                            ?>

                      <a href="#" type="button" data-bs-toggle="modal" data-bs-target="#komentar<?php echo $data['fotoid'] ?>">
                                          <i class="fa-regular fa-comment m-1" style="color: yellow;"></i>
                                      </a>
                <?php
                $jmlkomen = mysqli_query($koneksi, "SELECT * FROM komentarfoto WHERE fotoid='$fotoid' ");
                echo mysqli_num_rows($jmlkomen).' 条评论';
                ?>
            </div>
        </div>
                  ...
                  <div class="m-2">
                    <div class="overflow-auto">
                      ...
                      <div class="sticky-bottom">
                        <form action="../config/proses_komentar_copy.php" method="POST" >
                          <div class="input-group">
                            <input type="hidden" name="fotoid" value="<?php  echo $data['fotoid'] ?>" >
                            <input type="text" name="isikomentar" class="form-control" placeholder="添加评论">
                            <div class="input-group-prepend">
                              <button type="submit" name="kirimkomentar" class="btn btn-outline-primary" >发送</button>
                            </div>
                          </div>
                        </form>
                      </div>
